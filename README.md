...
Бот ведёт чат «как сервисный» — закрепляет постоянную сводку сверху и подчищает за собой эфемерные команды/ответы, чтобы история чата не превращалась в простыню. Для этого ему нужны права админа с двумя разрешениями:

- **`can_pin_messages`** — закрепляет «сводку» (single summary message) сверху чата. Без права функционал не ломается, но summary останется незакреплённым, а в логах раз в 30 секунд будет `failed to pin summary` — авто-восстанавливается, как только право выдано.
- **`can_delete_messages`** — удаляет транзитные ack-сообщения (свои `Сессия начата`, `Добавлен X`, `Трекинг включён`…) и пользовательские команды (`/start`, `/add`, нажатия reply-кнопок) **через 5 секунд** после завершения операции. Без права свои сообщения бот удалит всё равно (Telegram это разрешает), но команды пользователей останутся в чате — то есть подчистка будет частичной. Постоянные данные (сводка, live-локации пилотов, landing-алерты, ответы на `/list`/`/help`/`/status`) **никогда** не удаляются.
